...
namespace Module22Tp1.WebService
{
    ...
    {
        public String DataConnectionResource { get; }

        public WebServiceManager(string dataConnection)
        {
            DataConnectionResource = dataConnection;
        }
        ...
        protected async Task<TItem> HttpClientSender<TItem>(String url, TItem item, TItem result)
        {
            using (HttpClient client = new HttpClient())
            {
                client.BaseAddress = new Uri(DataConnectionResource);
        ...
        protected async Task<TItem> HttpClientPuter<TItem>(string url, TItem item, TItem result)
        {
            using (HttpClient client = new HttpClient())
            {
                client.BaseAddress = new Uri(DataConnectionResource);
        ...
        protected async Task<TResult> HttpClientDeleter<TItem, TResult>(string url, TItem item, TResult result)
        {
            using (HttpClient client = new HttpClient())
            {
                client.BaseAddress = new Uri(DataConnectionResource);
        ...
        protected async Task<TItem> HandleResponse<TItem>(TItem item, HttpResponseMessage response)
        {
